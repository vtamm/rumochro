...
class Fusion_WooCommerce {
	...
	/**
	 * Used to check if page contain checkout tabs.
	 *
	 * @since 3.9.2
	 * @access protected
	 * @var bool
	 */
	protected $is_checkout_tabs = false;
	...
	public function wp() {
		...
		$layout_content = function_exists( 'Fusion_Template_Builder' ) ? Fusion_Template_Builder()->get_override( 'content' ) : false;
		...
		if ( class_exists( 'WooCommerce' ) && isset( $post->post_content ) ) {
			...
			$check_content            = $layout_content ? $layout_content->post_content : $post->post_content;
			$this->is_checkout_tabs   = false !== strpos( $check_content, 'fusion_tb_woo_checkout_tabs' ) ? true : false;
		}
	}
	...
	/**
	 * Get is_checkout_tabs flag.
	 *
	 * @access public
	 * @since 3.9.2
	 * @return bool
	 */
	public function is_checkout_tabs() {
		return $this->is_checkout_tabs;
	}
