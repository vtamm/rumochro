...
( function( jQuery ) {
	...
	jQuery( document ).ready( function() {
		...
		var $disablePreview = jQuery( '.awb-modal-overlay' ),
			...
			AWBCriticalCSS = {
				...
				createCriticalCSS: async function() {
					...
					if ( true === self.ifGenerateMobileCSS() ) {
						...
						try {
							...
							self.pagesFailedErrors.push( e );
							criticalCSSData = false;
						}

						if ( Array.isArray( criticalCSSData ) ) { // if page successfully generated CSS.
					...
					function afterPageGenerated() {
						...
						if ( self.urlsLength === self.loopIndex ) {
							...
							if ( true === self.options.reloadPage && 0 === self.pagesFailedErrors.length ) {
								location.reload( true );
							} else {
								if ( self.pagesFailedErrors.length ) {
									self.updateMessageElement( pcString + criticalCSS.successWithFails );
									self.showPageErrors();
								}
				...
				showPageErrors: function() {
					...
					if ( this.pagesFailedErrors ) {
						html = '<div class="awb-critical-css-failed-wrapped">';

						this.pagesFailedErrors.forEach( ( error ) => {
							errorMessage = getMessageFromCSSError( error );
							if ( ! errorMessage.message ) {
								return;
							}

							// if the page isn't added, then add. this will prevent duplications from mobile and desktop.
							if ( ! pageUrls.includes( errorMessage.page ) ) {
								html += '<div class="awb-critical-css-failed-error">' + errorMessage.message + '</div>';
							}
							pageUrls.push( errorMessage.page );
						} );

						html += '</div>';
					}

					jQuery( '.awb-admin-modal-wrap .awb-admin-modal-content:visible' ).append( jQuery( html ) );
				},

				clearPageErrors() {
					jQuery( '.awb-admin-modal-wrap .awb-admin-modal-content .awb-critical-css-failed-wrapped' ).remove();
					this.pagesFailedErrors = [];
				}
		...
		jQuery( '#awb-critical-form' ).on( 'submit', function( event ) {
			...
			$disablePreview.show();
			AWBCriticalCSS.clearPageErrors();
			...
			jQuery.get(
				...
				function( response ) {
					...
					criticalCSSObj.createCriticalCSS().then( function() {
						...
						if ( AWBCriticalCSS.pagesFailedErrors.length ) {
							errorMessage = getMessageFromCSSError( AWBCriticalCSS.pagesFailedErrors[ 0 ] );
							if ( errorMessage.message ) {
								alert( errorMessage.message ); //eslint-disable-line no-alert
							}
						}
					} );
				}
		...
		jQuery( '#awb-enable-critical-css-btn' ).on( 'click', function() {
			var button  = jQuery( this ),
				wrapper = button.closest( '.awb-enable-critical-css' );
			...
			jQuery.post(
				ajaxurl,
				{
					action: 'awb_enable_critical_css',
					critical_nonce: jQuery( '#fusion-page-options-nonce' ).val()
				},
				function( response ) {
				...
				if ( ! response || '0' === parseInt( response ) ) {
		...
		function getMessageFromCSSError( error ) {
			var returnVal = {
					message: '',
					page: ''
				},
			...
			if ( ! error.urlErrors || 'object' !== typeof error.urlErrors ) {
				return returnVal;
			}

			// Get the message...
			for ( prop in error.urlErrors ) { // eslint-disable-line guard-for-in
				if ( ! error.urlErrors.hasOwnProperty( prop ) ) {
					continue; // eslint-disable-line no-continue
				}
				returnVal.message = error.urlErrors[ prop ].message;
				returnVal.page = prop;
				break;
			}
